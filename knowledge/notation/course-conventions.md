...
| Tidal tensor | In an observer's orthonormal frame, $\mathcal E_{ij} = c^2 R_{\hat\imath\hat 0\hat\jmath\hat 0}$, so geodesic deviation reads $d^2\xi^i/d\tau^2 = -\mathcal E_{ij}\xi^j$ and the Newtonian limit is $\mathcal E_{ij} = \partial_i\partial_j\Phi$ (outside a point mass, $\mathcal E = (GM/r^3)\,\mathrm{diag}(-2, 1, 1)$ along the radial and two transverse axes). Its trace is $\mathcal E_{ii} = c^2 R_{\mu\nu}u^\mu u^\nu = 4\pi G(\rho + 3p/c^2) - \Lambda c^2$ in general relativity. Never $K$ for the tidal tensor. |
