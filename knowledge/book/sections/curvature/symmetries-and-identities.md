...
- `twenty-of-256-slots` (flagship): The count made visible: the 256 slots of the spacetime Riemann tensor collapsing under each rule to a symmetric 6 by 6 array of pairs and then to 20 survivors.
...
- `four-legs-around-a-tiny-loop` (supporting): Shows the closing four-sided walk that a torsion-free connection guarantees: two routes from two arrows meet on the flat floor, miss by a gap of order size cubed on the ball, and keep pace with the area only on a made-up floor with twist.
- `cube-of-small-loops` (flagship): The cyclic identity as three trips around the faces at one cube corner, each carrying the edge its face lacks, with a badge saying when the reading direction makes the link a new one.
- `falling-ring-of-crumbs` (core): Tidal drift with no whirlpool: the mirror pairs of a falling cabin's three-by-three table of drifts, the net drift around a ring held at zero, and counters that read six numbers of twenty.
